...
export const ComparisonsSection = ({
	...
	return (
		<div className="space-y-4">
			<div>
				<h2 className="text-sm font-mono mb-2 text-neutral-300">{t("comparisons.title")}</h2>
				<p className="text-xs font-mono mb-3 text-neutral-500">{t("comparisons.description")}</p>
			</div>
				...
				<div className="flex flex-wrap gap-1">
					{availableLanguages.map((lang) => (
						<button
							...
							className={`px-2 py-0.5 border text-xs font-mono transition-colors ${selectedLanguages.includes(lang)
									? isDarkMode
						...
						>
							{selectedLanguages.map((langCode) => {
								...
								return (
									<div key={langCode}>
										<div
											className={`flex items-center gap-1 mb-1 px-1 py-0.5 text-xs font-mono ${isDarkMode ? "bg-neutral-900 text-neutral-500" : "bg-white text-neutral-600"
												}`}
										>
											{langCode}
										</div>
