...
export const logExecution = async (userId, codeId, success, output, error, executionTime) => {
  try {
    ...
    await db.query(
      ...
      [id, userId, codeId, success, output, error, executionTime],
    );
    return id;
  } catch (error) {
  ...
  try {
    ...
    const result = await db.query(
      `SELECT id, success, output, error, execution_time_ms, executed_at
       FROM execution_history WHERE code_id = $1
...
export const getUserExecutionHistory = async (req, res) => {
  try {
    ...
    const result = await db.query(
      `SELECT eh.id, eh.success, eh.output, eh.error, eh.execution_time_ms, eh.executed_at,
              c.id as code_id, c.title as code_title
       FROM execution_history eh
       LEFT JOIN codes c ON eh.code_id = c.id
       WHERE eh.user_id = $1
       ORDER BY eh.executed_at DESC LIMIT $2 OFFSET $3`,
  ...
  try {
    const result = await db.query(
      `SELECT COUNT(*) as total_executions,
              ...
              MAX(executed_at) as last_executed
       FROM execution_history WHERE user_id = $1`,
      [req.user.id],
    );

    const dailyStats = await db.query(
      `SELECT DATE(executed_at) as date, COUNT(*) as executions,
              SUM(CASE WHEN success = true THEN 1 ELSE 0 END) as successful
       FROM execution_history
       WHERE user_id = $1 AND executed_at >= NOW() - INTERVAL '30 days'
       GROUP BY DATE(executed_at) ORDER BY date DESC`,
      [req.user.id],
    );
