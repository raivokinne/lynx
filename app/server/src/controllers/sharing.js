  ...
  try {
    ...
    await db.query(
      `INSERT INTO shared_codes (id, code_id, share_token, is_public, expires_at)
       VALUES ($1, $2, $3, $4, $5)`,
      [id, codeId, shareToken, isPublic || false, expiresAt],
    );
  ...
  try {
    ...
    const result = await db.query(
      `SELECT id, share_token, is_public, view_count, expires_at, created_at
       FROM shared_codes WHERE code_id = $1`,
      [codeId],
    );
...
export const updateShare = async (req, res) => {
  try {
    ...
    const result = await db.query(
      `SELECT sc.id, c.user_id FROM shared_codes sc
       JOIN codes c ON sc.code_id = c.id
       WHERE sc.share_token = $1`,
      [token],
    );
    ...
    await db.query(
      `UPDATE shared_codes
       SET is_public = COALESCE($1, is_public), expires_at = COALESCE($2, expires_at)
       WHERE share_token = $3`,
  ...
  try {
    ...
    const result = await db.query(
      `DELETE FROM shared_codes sc
       USING codes c WHERE sc.code_id = c.id
       AND sc.share_token = $1 AND c.user_id = $2`,
      [token, req.user.id],
    );
  ...
  try {
    ...
    const result = await db.query(
      `SELECT c.id, c.title, c.language, c.description, c.created_at,
       ...
       JOIN codes c ON sc.code_id = c.id
       JOIN users u ON c.user_id = u.id
       WHERE sc.is_public = true AND (sc.expires_at IS NULL OR sc.expires_at > NOW())
